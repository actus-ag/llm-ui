<script lang="ts">
  ...
  import { findCompleteCsvBlock, findPartialCsvBlock, csvBlockLookBack } from '@llm-ui/svelte-csv';
  import CsvBlock from './CsvBlock.svelte';
  import TextBlock from './TextBlock.svelte';

  const exampleText = `Here's a list of fruits:
...
CSV blocks stream in item by item with smooth animation!`;
  ...
  const llmOutput = createLLMOutput({
    ...
    blocks: [
      {
        component: CsvBlock,
        findCompleteMatch: findCompleteCsvBlock({ type: 'fruits' }),
        findPartialMatch: findPartialCsvBlock({ type: 'fruits' }),
        lookBack: csvBlockLookBack({ type: 'fruits' }),
      },
      {
        component: CsvBlock,
        findCompleteMatch: findCompleteCsvBlock({ type: 'colors' }),
        findPartialMatch: findPartialCsvBlock({ type: 'colors' }),
        lookBack: csvBlockLookBack({ type: 'colors' }),
      },
    ],
    fallbackBlock: {
      component: TextBlock,
